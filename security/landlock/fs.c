...
/* Landlock superblock private data */
struct landlock_superblock {
	atomic_t inode_refs;
};

static inline struct landlock_superblock *landlock_superblock(
...
/* Super-block allocation/free hooks */

static int hook_sb_alloc(struct super_block *sb)
{
	struct landlock_superblock *landlock_sb;

	landlock_sb = kzalloc(sizeof(*landlock_sb), GFP_KERNEL);
	if (!landlock_sb)
		return -ENOMEM;
	/* kzalloc zeroes memory, so inode_refs is already 0 */
	sb->s_security = landlock_sb;
	return 0;
}
...
static struct security_hook_list landlock_hooks[] __lsm_ro_after_init = {
	...
	LSM_HOOK_INIT(sb_alloc_security, hook_sb_alloc),
	LSM_HOOK_INIT(sb_free_security, hook_sb_free_security),
