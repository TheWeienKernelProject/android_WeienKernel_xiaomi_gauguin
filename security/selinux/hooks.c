				    ...
				    const struct super_block *newsb)
{
    ...
    struct superblock_security_struct *new = selinux_superblock(newsb);	char oldflags = old->flags & SE_MNTMASK;
...
{
	int rc = 0;
	const struct superblock_security_struct *oldsbsec = selinux_superblock(oldsb);
